...
namespace DAL
{
    public class ProductoDAL //CORREGIR STOCK (no esta en la BD) 
    {
        public static DataTable BuscarProducto(string nombre)
        {
            ...
            string query = string.Format("SELECT P.ID_PRODUCTO, P.NOMBRE AS NOMBRE_PRODUCTO, P.PRECIO_COMPRA, P.PRECIO_VENTA, C.NOMBRE AS CATEGORIA FROM PRODUCTO P INNER JOIN CATEGORIA C ON P.ID_CATEGORIA = C.ID_CATEGORIA WHERE P.NOMBRE = '{0}'", nombre);
